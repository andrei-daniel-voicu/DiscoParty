...
void Lab2::Init()
{
	...
	// Create a mesh box using custom data
	{
		vector<VertexFormat> vertices =
		{
			...
			VertexFormat(glm::vec3(1, 1,  0), glm::vec3(1, 1, 1), glm::vec3(0.2, 0.8, 0.6))
		};
...
{
	srand(time(NULL));
	if (meshes.find("circle") != meshes.end())
		delete(meshes["circle"]);
	vector<VertexFormat> vertices;
	vector<unsigned int> indices;

	vertices.push_back({ glm::vec3(0, 0, 0), glm::vec3(1, 1, 1), glm::vec3(0.2, 0.8, 0.6) });

	for (int i = 1; i < noSides + 2; i++)
	{
		float x, y;
		x = radius * cos(i * 2 * M_PI / noSides);
		y = radius * sin(i * 2 * M_PI / noSides);

		float r, g, b;
		r = (float)rand() / RAND_MAX;
		g = (float)rand() / RAND_MAX;
		b = (float)rand() / RAND_MAX;
		vertices.push_back({ glm::vec3(x, y, 0), glm::vec3(r, g, b), glm::vec3(0.2, 0.8, 0.6) });
	}
...
void Lab2::Update(float deltaTimeSeconds)
{
	...
	RenderMesh(meshes["cube_B"], shaders["VertexColor"], glm::vec3(0, 1.5f, 0), glm::vec3(0.25f));

	// TODO(student): Draw the tetrahedron
	RenderMesh(meshes["tetrahedron"], shaders["VertexColor"], glm::vec3(0.5f, 1.f, 0.5f), glm::vec3(1));

	// TODO(student): Draw the square
	RenderMesh(meshes["square"], shaders["VertexColor"], glm::vec3(0, 0, 0), glm::vec3(1));

	meshes["circle"]->SetDrawMode(GL_TRIANGLE_FAN);
	RenderMesh(meshes["circle"], shaders["VertexColor"], glm::vec3(-0.2f, 0.5f, 1), glm::vec3(0.5f));
