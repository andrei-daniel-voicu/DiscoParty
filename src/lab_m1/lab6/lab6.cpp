...
Mesh* Lab6::CreateMesh(const char* name, const std::vector<VertexFormat>& vertices, const std::vector<unsigned int>& indices) {
	...
	// Observatie experiment interschimbare pipe:
	// daca transmit culoarea pe pipe 1 si normala pe pipe 3, atunci acestea se vor afisa
	// invers, deoarece shaderul ramas nemodificat se asteapta sa primeasca culoarea pe pipe 3
	// si normala pe 1
