...
void Lab5::Init() {
	...
	projectionMatrix = glm::perspective(fovY, window->props.aspectRatio, near, far);
}
...
void Lab5::Update(float deltaTimeSeconds) {
	...
	{
		glm::mat4 modelMatrix = glm::mat4(1);
		modelMatrix *= glm::translate(modelMatrix, glm::vec3(1.5f, 0.5f, -1.f));
		modelMatrix *= glm::rotate(modelMatrix, RADIANS(60.f), glm::vec3(0, 1, 0));
		modelMatrix *= glm::scale(modelMatrix, glm::vec3(2, 2, 2));
		RenderMesh(meshes["box"], shaders["Simple"], modelMatrix);
	}
	{
		glm::mat4 modelMatrix = glm::mat4(1);
		modelMatrix = glm::translate(modelMatrix, glm::vec3(-1.4f, 0.f, -2.f));
